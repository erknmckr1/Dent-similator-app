...
export default function Sidebar({
  ...
  return (
    <div
      className={cn(
        // BACKGROUND: bg-sidebar (Koyu Antrasit)
        // TEXT: text-sidebar-foreground (Kırık Beyaz)
        // BORDER: border-sidebar-border (İnce gri çizgi)
    ...
    >
      ...
      <div className="flex items-center p-4 mb-4 h-16">
        <div className={cn("flex items-center gap-2 transition-all", isCollapsed ? "justify-center w-full" : "justify-between w-full")}>
            
            {/* Logo Alanı */}
            <Link href="/dashboard" className={cn("flex items-center gap-3 transition-opacity duration-300", isCollapsed && "hidden")}>
            ...
            <button 
                ...
                className={cn(
                    ...
                    isCollapsed ? "absolute -right-3 top-6 bg-sidebar border border-sidebar-border rounded-full shadow-md z-50 text-sidebar-foreground" : "ml-auto"
                )}
            >
            ...
            {routes.map((route) => {
                ...
                const LinkContent = (
                    <div className={cn(
                        ...
                        <route.icon className={cn("w-5 h-5 transition-colors", !isCollapsed && "mr-3")} />
                    ...
                    <div key={route.href}>
                        {isCollapsed ? (
                            <Tooltip>
                                ...
                                <TooltipContent side="right" className="bg-sidebar-accent text-sidebar-foreground border-sidebar-border font-medium">
                                    {route.label}
                                </TooltipContent>
                            </Tooltip>
                        ) : (
          ...
          <div className="p-3 animate-in fade-in duration-500">
            ...
            <div className="bg-sidebar-accent rounded-xl p-4 border border-sidebar-border shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-semibold text-sidebar-foreground">Paket Durumu</h3>
                </div>
                <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-xs text-muted-foreground">
                        <span>Kalan</span>
                        <span className="text-sidebar-foreground font-medium">{creditCount}/{CREDIT_LIMIT}</span>
                    </div>
                    {/* PROGRESS BAR: Gösterge otomatik olarak 'primary' rengini (Bronz) alır */}
                    <Progress value={(creditCount / CREDIT_LIMIT) * 100} className="h-2 bg-sidebar-border" />
                </div>
            </div>
          </div>
      )}
      ...
      <div className="p-3 mt-auto border-t border-sidebar-border">
         <button className={cn("flex items-center w-full rounded-lg hover:bg-sidebar-accent transition p-2 group", isCollapsed ? "justify-center" : "gap-x-3")}>
            ...
            {!isCollapsed && (
                <>
                    <div className="flex flex-col items-start overflow-hidden">
                        <span className="text-sm font-medium text-sidebar-foreground truncate w-32 text-left">Dr. Ahmet Y.</span>
                        <span className="text-[10px] text-muted-foreground">Premium</span>
                    </div>
                    <LogOut className="w-4 h-4 ml-auto text-muted-foreground group-hover:text-sidebar-primary transition-colors" />
                </>
            )}
         </button>
      </div>
